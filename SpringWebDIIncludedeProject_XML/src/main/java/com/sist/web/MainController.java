 ...
 * 						변수의 초기화
 * 						----------
 * 						1. 명시적 초기화 => 클래스 제작시에 변수에 값을 직접 채워주는 방식
 * 						public class A
 * 						{
 ...
 * 			*** 요청을 받는 경우 (요청 데이터 전송)
 * 				=> request 안에 담겨서 돌아온다 => request.getParameter();
 * 					** request는 사용자 정보 (IP, PORT) 가지고 있다 (보안상 문제)
 * 						=> 가급적이면 사용 x
 * 						=> 사용자 정의 메소드를 만들 때 매개변수를 설정하면 DispatcherServlet에 의해
 * 							매개변수가 채워진다
 * 						=> JSP 결과값은 전송 => Model
 * 			Model을 작성
 * 			1) @Controller 설정해야 메모리 할당이 가능 (메소드 찾는 경우)
 * 			2) 메소드
 * 				= 리턴형 : String , void
 * 						 ------- 화면 이동에 관련된 JSP 정보 / 리다이레그 정보
